...
EventLoop::BasicRule::BasicRule(size_t s_category_id, InterestT s_interest,
    ...
    : category_id(s_category_id), interest(move(s_interest)),
...
EventLoop::FDRule::FDRule(BasicRule &&base, FileDescriptor &&s_fd,
                          ...
                          CallbackT s_error)
    : BasicRule(base), fd(move(s_fd)), direction(s_direction),
      cancel(move(s_cancel)), error(move(s_error)) {}

EventLoop::RuleHandle
EventLoop::add_rule(size_t category_id, FileDescriptor &fd, Direction direction,
